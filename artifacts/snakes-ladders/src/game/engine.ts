 ...
 */
import { JUMPS, LAST } from "./board.ts";
...
export const DEFAULT_RULES: Rules = { exactFinish: false, sixAgain: true };

export function newGame(specs: PlayerSpec[], rules: Rules = DEFAULT_RULES): GameState {
 ...
 * landed on has one.
 */
export function destination(from: number, roll: number, rules: Rules): { steps: Step[]; to: number } {
  const steps: Step[] = [];
...
export function playTurn(state: GameState, roll: number): TurnResult {
  ...
  const { steps, to } = destination(mover.pos, roll, state.rules);
  const players = state.players.map((p, i) => (i === idx ? { ...p, pos: to } : p));
  const won = to === LAST;
  const again = !won && state.rules.sixAgain && roll === 6;
  const next: GameState = {
    ...state,
    players,
    turn: won || again ? idx : (idx + 1) % players.length,
    phase: won ? "won" : "roll",
    winner: won ? idx : null,
    lastRoll: roll,
    rolls: state.rolls + 1,
  };
  return { state: next, player: idx, roll, steps, again };
}
